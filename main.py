...
def db_query(sql, params=None):
    try:
        ...
        if isinstance(result, list) and len(result) > 0:
            first = result[0]
            if "results" in first: return first["results"]
        return None
...
def get_users_count():
    try:
        ...
        if result and "rows" in result and len(result["rows"]) > 0: return result["rows"][0][0]
    except: pass
    return 0
...
def get_all_users():
    try:
        ...
        users = []
        if result and "rows" in result:
            for row in result["rows"]: users.append({"user_id": row[0], "full_name": row[1] or "بی‌نام", "username": row[2] or "", "join_date": row[3] or ""})
        return users
    except: return []
...
def detect_type(text):
    for protocol in ['vmess://', 'vless://', 'trojan://', 'hysteria2://', 'hysteria://', 'tuic://', 'ss://', 'ssr://']:
        if protocol in text.lower(): return "v2ray"
    return "proxy"
...
async def handle_channel_post(message: Message):
    ...
    if not current_operator["value"]: return
    ...
    if proxy_links:
        for link in proxy_links: save_to_db({"id": message.message_id, "text": link, "date": message.date, "type": "proxy", "operator": op, "file_id": None, "file_name": ""})
    elif v2ray_links:
        for link in v2ray_links: save_to_db({"id": message.message_id, "text": link, "date": message.date, "type": "v2ray", "operator": op, "file_id": None, "file_name": ""})
...
async def stats_details(c: types.CallbackQuery):
    ...
    for i, u in enumerate(users, 1):
        ...
        if len(txt) + len(line) > 4000: msgs.append(txt); txt = line
        else: txt += line
...
async def support_recv(message: Message, state: FSMContext):
    if message.text == "/cancel": await state.clear(); return await message.answer("❌ لغو شد.", reply_markup=get_main_menu())
    u = message.from_user
    try:
    ...
    except: await message.answer("❌ خطا در ارسال.")
    await state.clear()
